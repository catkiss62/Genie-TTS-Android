...
状态：v0.6.4 是模型与播放的收口基线；v0.7.0 增加 LLM 真流式联调；v0.7.1 在不改变采样和推理参数的前提下增加“首段即时、后续积压短句拼合”、统一单 AudioTrack 连续播放，并用第二套 V2 权重验证多语音包切换。测试引擎基于 Genie-TTS v2.0.2、GPT-SoVITS V2 权重和 Android ONNX Runtime；目标设备已完成中、英、日三语与约 500 字符分段播放真机验证。
