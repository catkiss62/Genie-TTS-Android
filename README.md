...
自由输入不是零特征回退：APK 内置动态 INT8 Chinese RoBERTa、BERT WordPiece 词表，以及由 Genie 原版中文 G2P 离线整理的常用词音素词典。自由输入首次生成会释放 TTS 会话、运行中文前处理和 RoBERTa，再重新加载 TTS 模型，以控制峰值内存；相同文本会缓存前处理结果。当前自由输入限制为 80 个规范化中文字符，暂不支持英文。
