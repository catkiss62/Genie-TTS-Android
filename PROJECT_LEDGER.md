...
最后更新：2026-09-07 · 当前测试版：v0.6.0 · 仓库：`catkiss62/Genie-TTS-Android`

## 当前接班区

项目已经证明 Genie-TTS v2.0.2 的 GPT-SoVITS V2 权重可以在 Android ARM64 上使用 ONNX Runtime 完整推理，中文音质可用，恬豆模型也已确认具备中、英、日三语能力。当前主音色为候选 1，推理固定 CPU 8 线程。

v0.6.0 的目标是把 v0.5.0 的英日固定音素验证升级为真正的手机端动态前端：中文允许夹杂英文，纯英文与纯日语使用独立模式。英日不加载额外 BERT；中文只运行现有 Chinese RoBERTa。测试完成并确认动态发音后，才进入 AI 伴侣工程接入。
...
1. 安装 APK，保持候选 1。
2. 先运行 4 条“中文＋英文”预设，确认 `DeepSeek`、`token`、API/GPT、ONNX/CPU 发音和中文衔接。
3. 再运行 4 条纯英文预设。复制一份报告，确认“动态前端校验”为黄金音素完全一致。
4. 首次点击日语会多出词典释放时间；等待完成后再运行同一句，第二次才代表热启动前端。
5. 运行 4 条纯日语预设并复制报告，检查黄金音素校验、自然度和音色保持。
6. 若只有某个词发音不理想，优先补热词；不要为了个别词修改全局采样参数。

## AI 伴侣接入待办

在 v0.6.0 真机确认后按以下顺序继续：
