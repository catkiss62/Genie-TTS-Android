...
最后更新：2026-09-23 · 当前开发版：v0.8.3（首尾对照与浮点/内存优化） · 历史稳定基线：v0.6.4 · 仓库：`catkiss62/Genie-TTS-Android`

## 当前接班区

项目已经证明 Genie-TTS v2.0.2 的 GPT-SoVITS V2/V2Pro 权重可以在 Android ARM64 上使用 ONNX Runtime 完整推理。历史恬豆基线已完成三语和长文本验证；从 v0.7.7 起当前测试运行时只保留小酒狐 V2Pro 四候选，v0.7.8 在不改模型和采样的前提下加入五档互斥 CPU 调度实验，v0.7.9 修正完整 APK 的资源表封装，v0.8.0 为五档加入统一的一键单次对比报告，v0.8.1 修正持续性能模式的线程错误并把短句采样升级为 219 字连续分段基准，v0.8.2 证明映射复用与动态分块没有有效收益，v0.8.3 改为首尾冻结对照并直接筛选 Decoder/VITS 浮点和内存执行选项。
...
- 真机依据：第一轮有效连续报告中自动核亲和 RTF 1.038、基准 8 线程 RTF 2.299，快 54.8%；手机冷却后的第二轮中自动核亲和 RTF 0.949、基准 8 线程 RTF 2.079，快约 54.4%。第二轮自动档核心推理 37,201 ms、连续墙钟 44,707 ms、39.200 秒音频，最小推算缓冲 -3,879 ms。两轮方向一致，已足够确认自动核亲和不是偶然波动。
- 冻结对照：第一档保留 v0.8.1 的 CPU EP、`threads=0`、顺序图、inter-op 1、线程忙等开启和原始 Decoder 循环；任何新优化不得进入该档，报告统一相对它计算差异。
...
- 验证与交付：分段器与连续报告纯 Kotlin/JUnit 共 13 项本地测试通过；全项目共 21 项测试。公开分支 `agent/v081-continuous-performance-fix`、草稿 PR #13、远端源码提交 `da0df53`；GitHub Actions 第 45 次运行的打包工具语法、Android/Kotlin 编译、全量单测、瘦 APK、安装兼容门禁和打包工具上传全部通过。完整 APK `Genie-TTS-Android-v0.8.1-continuous-performance-test.apk` 为 355,373,174 字节，SHA-256 `4a8fd169551bb3a0761dbbe9e510f6c5702c58dad88264676468dc50e6b53721`；15 个 CI 非 META 核心条目逐字节一致，49 个酒狐资源条目与 v0.8.0 已验证包逐一一致，恬豆/乐奈/mao/旧 `benchmark` 条目为 0；`resources.arsc` 保持未压缩，ZIP、4 字节/16 KiB zipalign、AArch64 ELF、安装门禁及 v2/v3 签名均通过，证书继续沿用 AOSP testkey `a40da80a59d170caa950cf15c18c454d47a39b26989d8b640ecd745ba71bf5dc`。未合并 `main`，未发布正式 Release。

### v0.8.0 一键五档单次性能对比

- 目的：用户已观察到部分性能档明显更快，但逐档手动切换、等待、复制多份报告难以形成可比证据。新增“运行五档单次对比（不播放）”和“复制五档单次对比报告”，一次完成整套采样与复制。
- 测试定义：按 `基准8线程 → 自动核亲和 → 均衡6线程 → 图并行4×2 → 长时稳态8` 固定顺序运行；每档都先卸载四个 ONNX 会话，再执行一次独立冷加载和一次相同的预计算中文“普通闲聊”推理。资源释放不计入报告，模型冷加载与端到端等待分别记录。
- 公平性：五档共用当前选择的小酒狐参考音色、同一预设音素/BERT 张量和相同模型；不创建 `AudioTrack`，语速、音调、高频柔化和播放增益均不参与。测试结束恢复用户原先选择的性能档，但保持会话卸载，避免把最后一档残留成下一次测试的热缓存。
- 报告：逐档记录完整配置、持续性能模式实际请求、前后温控、模型加载、张量读取、Encoder、首步/自回归 Decoder、VITS、核心推理、端到端、音频时长、RTF、语义序列、波形摘要和 PSS；末尾按核心 RTF 排名并计算相对基准快慢，同时检查五档语义序列是否一致。
- 容错：单个实验档建会话或推理失败时，记录档位、温控和截断错误栈并继续剩余档位；只有用户点击停止才取消整套测试，避免某一档异常导致已完成数据无法复制。
- 判断边界：单次结果会受温控、后台负载和固定顺序影响，只用于筛选候选档；收到报告后应挑最快两档做重复测试，不能仅凭一次结果直接写入正式默认值。
- 验证与交付：新增报告格式/排名/相对基准/语义差异/单档失败继续 3 项 JUnit 测试；全项目共 19 项测试。公开分支 `agent/v080-one-tap-performance-report`、草稿 PR #12、远端源码提交 `e83e434`；GitHub Actions 第 43 次运行的 Python 语法、Android/Kotlin 编译、全部单测、瘦 APK、安装兼容门禁和打包工具上传均通过。完整 APK `Genie-TTS-Android-v0.8.0-one-tap-performance-test.apk` 为 355,926,086 字节，SHA-256 `5c8c23ef549bf6cfa7553301ac25595130cf3e3eecd8b7b237a7370b3767f17a`；15 个 CI 非 META 核心条目逐字节一致，酒狐清单 48 项逐一通过字节数与 SHA-256，恬豆/乐奈条目为 0；未压缩 `resources.arsc`、ZIP、4 字节/16 KiB zipalign、arm64 ELF 与 v2/v3 签名全部通过，证书继续沿用 AOSP testkey。
...
- 定义：本项目没有权限也不尝试修改 CPU 电压、频率或系统 governor；界面沿用用户容易理解的“超频”说法，但实际测试的是 ONNX Runtime 与 Android 提供的应用层线程/持续性能调度。
- UI：加入五个互斥按钮，避免多个开关产生无法复现的任意组合。`基准8线程` 保持 v0.7.7；`自动核亲和` 使用 intra-op 0，让 ORT 按物理核建池并自动亲和；`均衡6线程` 测试减少异构核和内存带宽争用；`图并行4×2` 使用 intra-op 4、inter-op 2 与 parallel execution；`长时稳态8` 保持 8 线程顺序图，并仅在设备声明支持时调用 Android Sustained Performance Mode。
- 公平性：五档都保持 CPU EP、`ALL_OPT` 和线程忙等，不重新开放已出现异常短音频的 XNNPACK，也不重新开放已证实无收益的 NNAPI VITS。中文 INT8 RoBERTa 现在跟随所选档位，不再始终占用设备全部逻辑核。
- 生命周期：切档立即停止播放、卸载四个声学会话、关闭中文 RoBERTa 会话并清空特征缓存；下一次运行冷加载新线程池。切档同时清除旧性能报告，避免旧结果被误贴新档标签。
...
### v0.7.7 小酒狐单模型与沉浸房间长文本基准

- 范围：本轮只删除恬豆并建立酒狐长文本基准，不修改 CPU 线程数、ONNX SessionOptions、Decoder/VITS 图、采样参数或任何“超频”策略。
- 资源：运行时语音包只剩 `jiuhu`，唯一 APK 命名空间为 `benchmark_jiuhu`；构建前门禁发现 `app/src/main/assets/benchmark` 就直接失败，防止完整 APK 误带恬豆模型。
- 覆盖安装清理：启动后后台删除 `files/genie-benchmark` 下名称包含 `tiandou` 的旧解压目录；若共享 RoBERTa 仍只在旧目录，先把模型与 SHA-256 标记迁到 `shared` 再删除，且始终保留 `voices/jiuhu`，避免省空间时误删仍需复用的中文前端。
- 长文本：移除旧固定约 500 字夹具，中文、英文、日文改用约 1000 字 `DialogueFixtures`；固定长文本与模拟/DeepSeek 真流式共用同一文本，便于后续提速前后直接比较。
- 分段依据：读取 AI 伴侣 `agent/v04190-cedar-save-slots-custom-final-model` 的沉浸房间实现。其链路先按动作/对白语义闭合，再由句末标点入队；首句立即提交，后续只拼合 TTS 忙碌期间已积压的完整短句；普通逗号不触发提前切分，异常无标点长串才按中日 54/英文 110 字兜底。该方案能兼顾首音频延迟、语气完整性和声学输入上限，适合作为酒狐性能基准。
- 播放保持：仍为单 `AudioTrack.MODE_STREAM`、首段 1000 ms 预填充、播放前段时串行生成后段、无固定段间 sleep；酒狐语速、音调和高频柔化继续生效且跨段保持滤波状态。
- 本地验证：分段、积压拼合、单模型目录和旧缓存清理共 10 项 JUnit 测试通过；三语长文本分别为中文 873 字/23 段、英文 1191 字符/14 段、日文 944 字符/25 段，首段分别为 26/75/32 字符，最长段均未超过 54/110/54 上限；Python 打包工具语法检查通过。当前容器无法通过 Gradle 拉取 Android 插件，因此完整 Android 编译留给后续获授权后的 GitHub Actions。本轮未获新的推送/构建授权，不推送公开分支、不生成或上传 APK。
...
| 项目 | 结论 |
|---|---|
| 当前模型 | 只保留小酒狐 V2Pro 的一套 GPT/SoVITS 配对权重；四候选是四组独立参考提示，不是四套模型 |
| 训练数据集 | 只用于训练/重训，不是 Android 运行时依赖，不放入 APK |
| 酒狐候选映射 | `daily`=便当与备用工具；`gentle`=如梦的日子；`cute`=献给主人；`lively`=原始 `idle50`；四项共用声学权重但不得共用参考张量 |
| 已移除模型 | 恬豆、奶油、乐奈均只保留历史记录，不进入 v0.7.8 运行时或 APK 资源清单 |
| 后端 | v0.7.8 只比较五种 CPU 调度档；XNNPACK 曾产生异常短音频，NNAPI VITS 没有收益，均不重开 |
