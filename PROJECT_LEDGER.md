...
最后更新：2026-09-07 · 当前测试版：v0.6.2 · 仓库：`catkiss62/Genie-TTS-Android`
...
v0.6.1 的中文 505 字长文本真机回归失败：用户确认 v0.5.0 几乎没有停顿，v0.6.1 却在几乎每句后停顿数秒。报告记录聚合 RTF 2.475、15/15 后续段迟到、15 次 AudioTrack underrun、最小缓冲余量 -113249 ms。v0.6.2 已恢复 v0.5.0 的固定 1 秒预填充，取消启动后台预热，并把英文 CMUdict、日语 OpenJTalk 改为按语言懒加载；当前等待真机隔离验证，不能提前宣称回退已经解决。
...
| v0.6.2 | 回归隔离版：固定 1 秒预填充、取消后台模型预热、英日前端懒加载；加入中/英/日约 500 字符独立长文本试听，报告只保留最后一次结果；待真机验证 |
...
4. 中文恢复后，分别运行约 500 字符英文和约 500 字符日文试听。长文本报告只保存最后一次；若某种语言停顿或听感异常，在测试下一语言前立即复制该报告；全部正常则不必发送报告。
5. 约 1000 字模拟 DeepSeek 真流式继续延后；不得在回退未解除时叠加新的流式调度。
